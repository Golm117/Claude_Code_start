...
Variables consumed:
  - {{project_name}}
  - {{approved_stack}}
  - {{agent_roster}} (rendered as a bulleted list of agent names with one-line role descriptions)

This is the project-level coordination protocol. It tells humans, Claude Code, and
any web-chat Claude collaborator how to work together on the repo.
-->
...
```bash
git worktree add ../{{project_name}}-wt-<area> feature/<branch>
```
...
- Any task touching mandatory-reviewer triggers (see each reviewer agent's intro for their trigger list) routes through that reviewer before merge.
- Any non-trivial change routes through review (whichever agent in your roster owns review) before merge.
...
The approved stack for this project is `{{approved_stack}}`. Anything outside that list requires human approval and at least one alternative being considered.
